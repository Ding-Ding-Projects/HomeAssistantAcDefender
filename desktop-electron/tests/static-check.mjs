...
const required = ["/api/status", "/api/target", "/api/defender", "/api/thermostat/off", "AntiforgeryToken"];
for (const marker of required) if (!main.includes(marker)) throw new Error(`Missing API/auth marker: ${marker}`);
for (const marker of ["Ctrl+Shift+F", "bilingual", "funnyEnglish", "funnyCantonese", "SEARCH / REGEX BUILDER"]) if (!app.includes(marker)) throw new Error(`Missing controller feature: ${marker}`);
