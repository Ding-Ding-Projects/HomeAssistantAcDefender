...
function Notifications({ snapshot, onLoad, onAction, t }: { snapshot: NotificationSnapshot | null; onLoad: () => Promise<void>; onAction: (id: string, action: "read" | "dismiss" | "restore") => Promise<void>; t: (value: Copy) => string }) {
  ...
  const filtered = items.filter((item) => {
    const haystack = `${item.title || ""} ${item.level || ""} ${item.message || ""}`;
    if (regex) return regex.test(haystack);
    return !query.trim() || haystack.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase());
  });
  function usePattern(next: string, nextFlags: string) { try { const cleanFlags = nextFlags.replace("g", ""); setRegex(new RegExp(next, cleanFlags)); setPattern(next); setFlags(nextFlags); setQuery(next); } catch { setRegex(null); } }
  return <section className="stack-page"><div className="page-heading"><div><h2>{t(labels.notifications)}</h2><p className="supporting">Reviewable server history; dismissing never deletes the record.</p></div><button className="button button-tonal" onClick={() => void onLoad()}>{t(labels.refresh)}</button></div><div className="card"><div className="eyebrow">{snapshot?.unreadCount ?? 0} UNREAD · {snapshot?.activeCount ?? items.length} ACTIVE</div><div className="search-row"><input aria-label="Search notifications" value={query} onChange={(e) => { setQuery(e.target.value); setRegex(null); }} placeholder="Search notification history" /><RegexBuilder open={builderOpen} onOpenChange={setBuilderOpen} pattern={pattern} flags={flags} onPatternChange={(value) => { setPattern(value); try { setRegex(value ? new RegExp(value, flags.replace("g", "")) : null); } catch { setRegex(null); } }} onFlagsChange={(value) => { setFlags(value); try { setRegex(pattern ? new RegExp(pattern, value.replace("g", "")) : null); } catch { setRegex(null); } }} onUse={usePattern} title="Notification search regex builder" /></div>{filtered.length ? <ul className="notification-list">{filtered.map((item: NotificationItem, index: number) => <li key={item.id || index} className={item.read ? "notification read" : "notification"}><div><strong>{item.title || item.level || "Notification"}</strong><p>{item.message || "No message"}</p><time>{item.timestamp ? new Date(item.timestamp).toLocaleString() : "—"}</time></div><div className="button-row">{item.id && !item.read && <button className="button button-tonal" onClick={() => void onAction(item.id!, "read")}>Mark read</button>}{item.id && !item.dismissed && <button className="button button-tonal" onClick={() => void onAction(item.id!, "dismiss")}>Dismiss</button>}{item.id && item.dismissed && <button className="button button-tonal" onClick={() => void onAction(item.id!, "restore")}>Restore</button>}</div></li>)}</ul> : <p className="empty-state">No notifications match this search.</p>}</div></section>;
}
